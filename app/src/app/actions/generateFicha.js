...
export async function generateFichaAction(formData) {
  try {
    ...
    if (!briefFile) {
      return { error: 'Brief de contenido (Word) requerido' };
    }
...
function buildDemoFicha(headerPhoto, partnerLogo) {
  return {
    ...
    intro: 'Un seguro que te entrega respaldo económico para tus consultas médicas, cirugías y exámenes ambulatorios.',
    keyData: [
    ...
    tables: [{
      ...
      columns: [
        ...
        { key: 's', label: 'Cobertura sin bonificación', width: '19.5%' },
        { key: 't', label: 'Tope por prestación', width: '14.4%' },
      ],
